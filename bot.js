...
bot.on("message", async message => {
	...
	if(command === `${prefix}mute`) {
		...
		if(toMute.highestRole.position >= message.member.highestRole.position) return message.channel.sendMessage("You can't mute that bitch")


		let role = message.guild.roles.find(r => r.name === "RevoBot Muted");
		if(!role){
			try{
